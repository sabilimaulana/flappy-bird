...
export class FlappyBirdLite extends GameBase {
  ...
  protected onLoad() {
    ...
    const addressConfig = {
      tonAddress: "kQB-7tmw6QX9eV7pod3HeI-6ldaYKgYui31f0-mgQVz72bLL",
      jettonAddress: "kQAcmdbPgT_m06SpZA0_60MbV7lgfww2yOoXS5vNoi8xBaB2",
    } as TonAddressConfig;
  ...
  private _createBird(): FBird {
    let bird: FBird = null;
    if (this.birdPrefab) {
      let birdNode: Node = PoolManager.Instance().getNode(
        this.birdPrefab,
        this.birdRoot
      );
      if (birdNode) {
  ...
  private _onTouchStart(event: EventTouch) {
    ...
    FBRecordManager.Instance().addRecord(
      FBAction.TOUCH_DOWN,
      FBInputType.TOUCH_OR_MOUSE
    );
  }
